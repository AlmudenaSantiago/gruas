...
public class GenerarFacturaPDF {
    ...
    public GenerarFacturaPDF(Pedido pedido,Double cantidadEntregada,Cliente cliente, File path) {
        this.pedido = pedido;
        ...
        try {
          ...
          // String path = ClassLoader.getSystemClassLoader().getResource(".").getPath();
           ...
           table.setWidthPercentage(100);
            ...
            System.out.println("el descuento es:" + pedido.getDescuento());
            if (!"0".equals(pedido.getDescuento())) {
                 PdfPCell celdaDescuento = new PdfPCell(new Paragraph("Descuento"));
                celdaDescuento.setColspan(4);
                celdaDescuento.setBorderColor(BaseColor.BLACK);


                Paragraph importeDescuento = new Paragraph(pedido.getDescuento() + " €");
                importeDescuento.setFont(FontFactory.getFont("Century",Font.BOLD));
                PdfPCell celdaImporteDescuento = new PdfPCell(importeDescuento);
                
                 table.addCell(celdaDescuento);
                 table.addCell(celdaImporteDescuento);
            ...
            Double importeTotalConDescuento = pedido.getImporte() - Double.parseDouble(pedido.getDescuento());
            Paragraph importeTotal = new Paragraph( importeTotalConDescuento + " €");
