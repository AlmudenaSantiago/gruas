...
public class EliminarDistancia {

   Distancia d;
   
   
    public EliminarDistancia(Distancia d) {
       this.d= d;
    }

    public void eliminar () throws Exception {
        DistanciaParserJson parser = new DistanciaParserJson();
        String stringjson = parser.devuelveJsonDeDistancia(d);  
        String stringRespuestaPOST = conexionPOST(stringjson);
        if (stringRespuestaPOST == null)   JOptionPane.showMessageDialog (null,"No se ha podido eliminar la distancia");
     }
     ...
     private String conexionPOST(String datos) {
        ...
        try {
            // creamos la conexion
            url = new URL("http://localhost/gruas/gruas/api/eliminarDistancia");
